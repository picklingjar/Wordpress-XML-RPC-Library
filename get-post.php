...
//metaweblog.getPost
function wordpress_get_post($xmlrpcurl, $postid, $username, $password, $proxyipports = ""){
	global $globalerr;
	...
	$msg = new xmlrpcmsg("metaWeblog.getPost",$params);
...
/*
blogger.getPost
function wordpress_get_post($xmlrpcurl, $postid, $username, $password){
	$client = new xmlrpc_client($xmlrpcurl);
	$unused = "";
	$params[] = new xmlrpcval($unused);	//unused
	$params[] = new xmlrpcval($postid);	//post ID
	$params[] = new xmlrpcval($username); //your wordpress login
	$params[] = new xmlrpcval($password); //your wordpress password
	$msg = new xmlrpcmsg("blogger.getPost",$params);
	$r = $client->send($msg);
	if($r === false){
		return(false);
	}
