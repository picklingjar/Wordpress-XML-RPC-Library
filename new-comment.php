...
//if no username password, use author name
function wordpress_new_comment($xmlrpcurl, $username = '', $password = '', $blogid = 0, $postid, $author, $authoremail, $authorurl, $content, $commentparent = 0, $proxyipports = ""){
	global $globalerr;
	...
	$rpcstruct= new xmlrpcval(
		array(
			"author" => new xmlrpcval($author, "string"),
			"author_email" => new xmlrpcval($authoremail, "string"),
			"author_url" => new xmlrpcval($authorurl, "string"),
			"comment_parent" => new xmlrpcval($commentparent, "int"),
			"content" => new xmlrpcval($content, "string"),
  		),
		"struct");

	$params[] = $rpcstruct;
	$msg = new xmlrpcmsg("wp.newComment",$params);
