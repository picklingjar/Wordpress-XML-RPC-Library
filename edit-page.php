...
function wordpress_edit_page($xmlrpcurl, $pageid, $username, $password, $blogid = 0, $slug = "", $wp_password="", $parent_id = "", $page_order= "0", $author_id = "0", $title, $content, $excerpt, $text_more, $comments = "0", $pings = "0", $date_created, $customfields = '', $publish=1, $proxyipports = ""){
	global $globalerr;
	$client = new xmlrpc_client($xmlrpcurl);
    $client->setSSLVerifyPeer(false);
	$params[] = new xmlrpcval($blogid);
	$params[] = new xmlrpcval($pageid);
	...
	$rpcstruct= new xmlrpcval(
		...
		"struct");
	$params[] = $rpcstruct;
	$params[] = new xmlrpcval($publish,'boolean'); //your wordpress password
	
	$msg = new xmlrpcmsg("wp.editPage",$params);
